...
const steps = {
  es: [
    {
      mode: 'iex', iexState: 'prompt',
      title: '¿Qué es la IEx?',
      explanation: 'IEx (Interactive Elixir) es la consola interactiva que viene con Elixir. La abres con iex y puedes escribir cualquier expresión al instante: llamar una función y ver qué retorna, crear una variable y usarla en la siguiente línea, probar un módulo, hacer un cálculo. Sin archivos, sin compilación — escribes y ves el resultado de una.',
    },
    {
      mode: 'iex', iexState: 'command',
      title: 'Escribimos en el prompt',
      explanation: 'Escribir IO.puts("Hello, World!") en la IEx es exactamente lo mismo que hacerlo en un archivo. El módulo IO, la función puts, el string entre comillas — todo igual. La diferencia es que aquí el resultado aparece de una, en la misma pantalla.',
    },
    {
      mode: 'iex', iexState: 'output',
      title: 'La IEx responde con dos líneas',
      explanation: '"Hello, World!" es el texto que IO.puts imprimió. Pero la IEx también muestra :ok en azul — ese es el valor de retorno de la función. En Elixir toda expresión devuelve un valor. :ok es un átomo: un identificador inmutable que aquí significa que la operación fue exitosa.',
    },
    {
      mode: 'transition',
      title: 'Ahora, en un archivo',
      explanation: 'La IEx es perfecta para experimentar, pero el código que quieres conservar y reutilizar vive en archivos .ex. Vamos a hacer exactamente el mismo Hola Mundo — esta vez escrito en un archivo y ejecutado desde la terminal.',
    },
    {
      mode: 'file',
  ...
  en: [
    {
      mode: 'iex', iexState: 'prompt',
      title: 'What is IEx?',
      explanation: 'IEx (Interactive Elixir) is the interactive console that ships with Elixir. Open it with iex and you can type any expression instantly: call a function and see what it returns, create a variable and use it on the next line, try a module, run a quick calculation. No files, no compilation — just type and see the result.',
    },
    {
      mode: 'iex', iexState: 'command',
      title: 'We type at the prompt',
      explanation: 'Typing IO.puts("Hello, World!") in IEx is exactly the same as writing it in a file. The IO module, the puts function, the string in quotes — all identical. The difference is that here the result appears instantly, right on the same screen.',
    },
    {
      mode: 'iex', iexState: 'output',
      title: 'IEx responds with two lines',
      explanation: '"Hello, World!" is the text IO.puts printed to the screen. But IEx also shows :ok in blue — that\'s the return value of the function. In Elixir every expression returns a value. :ok is an atom: an immutable identifier that here means the operation was successful.',
    },
    {
      mode: 'transition',
      title: 'Now, in a file',
      explanation: 'IEx is perfect for exploration, but code you want to keep and reuse lives in .ex files. Let\'s do the exact same Hello World — this time written in a file and executed from the terminal.',
    },
    {
      mode: 'file',
      title: 'An empty file',
...
const IexPanel = ({ iexState }) => (
  <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-900 dark:bg-black/70 backdrop-blur-md overflow-hidden">
    <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/10 bg-white/5">
      <span className="inline-flex items-center justify-center px-1.5 py-0.5 rounded border border-white/20 bg-white/10 text-xs font-mono text-white/50 leading-none">&gt;_</span>
    ...
    <div className="h-36 p-4 font-mono text-sm">
      ...
      <p className="text-white/20 text-xs mb-3">Interactive Elixir · Ctrl+C twice to exit</p>
      {iexState === 'prompt' && (
  ...
  <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-md overflow-hidden">
    <div className="h-36 flex items-center justify-center gap-8">
      <div className="text-center">
        ...
        <p className="text-xs text-muted">{lang === 'es' ? 'exploración' : 'exploration'}</p>
      </div>
      <div className="text-slate-300 dark:text-white/20 text-2xl">→</div>
      <div className="text-center">
        <p className="font-mono text-sm text-amber-500 dark:text-amber-400 mb-1">hello.ex</p>
        <p className="text-xs text-muted">{lang === 'es' ? 'persistencia' : 'persistence'}</p>
      </div>
    </div>
  </div>
)
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-slate-50 dark:bg-black transition-colors duration-300">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-6 py-12 max-sm:pb-24">
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          <div className="rounded-xl border border-violet-500/30 dark:border-violet-500/40 bg-violet-500/5 dark:bg-violet-500/15 backdrop-blur-sm overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-violet-500/15 dark:border-violet-500/30">
              <span className="text-[10px] font-semibold uppercase tracking-widest text-violet-500 dark:text-violet-400">IEx</span>
              <span className="text-[10px] text-violet-400/60">{lang === 'es' ? '· exploración interactiva' : '· interactive exploration'}</span>
            </div>
            <div className="p-3 font-mono text-xs space-y-0.5">
              <p className="text-slate-400 dark:text-violet-400/50 text-[10px] mb-1.5">$ iex</p>
              <div>
                <span className="text-violet-500 dark:text-violet-400">iex(1)&gt;</span>
                {' '}
              ...
              <p className="text-emerald-600 dark:text-emerald-400">Hello, World!</p>
          ...
          <div className="rounded-xl border border-amber-500/30 dark:border-amber-500/40 bg-amber-500/5 dark:bg-amber-500/12 backdrop-blur-sm overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-amber-500/15 dark:border-amber-500/30">
              <span className="text-[10px] font-semibold uppercase tracking-widest text-amber-600 dark:text-amber-500">hello.ex</span>
              <span className="text-[10px] text-amber-500/60">{lang === 'es' ? '· código persistente' : '· persistent code'}</span>
            </div>
            <div className="p-3 font-mono text-xs space-y-0.5">
              ...
              <p className="text-slate-400 dark:text-white/30 text-[10px]">$ elixir hello.ex</p>
              <p className="text-emerald-600 dark:text-emerald-400">Hello, World!</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-4">
          {current.mode === 'iex' && (
            <IexPanel iexState={current.iexState} />
          )}
          {current.mode === 'transition' && (
            <TransitionPanel lang={lang} />
          )}
          {current.mode === 'file' && (
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-md overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-200 dark:border-white/10">
                  <span className="inline-flex items-center justify-center px-1.5 py-0.5 rounded border border-slate-300 dark:border-white/20 bg-slate-100 dark:bg-white/10 text-xs font-mono text-slate-500 dark:text-white/50 leading-none">&gt;_</span>
                  <span className="text-xs text-slate-400 dark:text-white/25 font-mono">hello.ex</span>
                </div>
              ...
              <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-900 dark:bg-black/70 backdrop-blur-md overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-200 dark:border-white/10 bg-white/5">
                  <span className="inline-flex items-center justify-center px-1.5 py-0.5 rounded border border-slate-300 dark:border-white/20 bg-slate-100 dark:bg-white/10 text-xs font-mono text-slate-500 dark:text-white/50 leading-none">&gt;_</span>
                  <span className="text-xs text-slate-400 dark:text-white/40 font-mono">terminal</span>
                </div>
                <div className="h-36 p-4 font-mono text-sm">
                  {current.showCommand
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            {current.mode === 'iex' && (
              <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-500 dark:text-violet-400 border border-violet-500/20">IEx</span>
            )}
            {current.mode === 'file' && (
              <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">hello.ex</span>
            )}
            <p
